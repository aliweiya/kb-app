...
function reload(me, myPara) {
	...
	//抓去数据
	fetch(me.data.api, { forward: 0 }, myPara).then(function (d) {
...
function init(me, atype) {
	...
	if (!g.comPostInfo.openid && userInfo) {
...
function upperLoad(me, myPara, cb) {
	...
	fetch(me.data.api, urlPara, myPara).then(function (d) {
		...
		if (me.data.newsListData.length && d.data && d.data.recommWording) {
			hideTopTip(me, d.data.recommWording);
		}
